...
#      Utf8Converter.convert_ka_to_en(self.read_attribute(:first_name)).gsub(/[^A-Za-z ]/,'').titlecase
  ...
  def self.api_v2_members(parliament_id=nil)
    x = []
    members = AllDelegate.select('id, first_name, started_at, ended_at, parliament_id').order("first_name asc")
    
    parliaments = Parliament.api_v2_parliaments
    parliament = nil

    # filter by parliament id if it is provided
    if parliament_id.present?
      parliament = parliaments.select{|x| x[:internal_id].to_s == parliament_id.to_s}.first
      if parliament.present?
        members = members.where(:parliament_id => parliament_id)
      end
    end
    ...
    if members.present?
      members.each do |member|
        ...
        if parliament.present?
          member_parliament = parliament
        else
          member_parliament = parliaments.select{|x| x[:internal_id] == member.parliament_id}.first
        end
        if member_parliament.present?
          h[:parliament] = member_parliament
        end
      end
    end
    return x
  ...
  def self.api_v2_member_votes(member_id, with_laws=false, with_law_vote_summary=false, passed_after=nil, passed_before=nil, made_public_after=nil, made_public_before=nil)
    ...
    if member_id.present?
      ...
      if !File.exists?(file_path)
        ...
        if member.present?
          ...
          # add parliament
          parl = Parliament.where(id: member.parliament_id).first
          h[:member][:parliament] = parl.to_hash if parl.present?
  ...
  def self.api_v2_all_member_votes(with_laws=false, with_law_vote_summary=false, passed_after=nil, passed_before=nil, made_public_after=nil, made_public_before=nil, parliament_id=nil)
    #create file name
    q = parse_query_params(with_laws, with_law_vote_summary, passed_after, passed_before, made_public_after, made_public_before, parliament_id)
    ...
    # if the file does not exist yet, create it
    if !File.exists?(file_path)
      members = api_v2_members(parliament_id)
      if members.present?
        File.open(file_path, 'a') {|f| f << '[' }
        members.each_with_index do |member, i|
          # create the member vote data
          member_file = api_v2_member_votes(member[:id], with_laws, with_law_vote_summary, passed_after, passed_before, made_public_after, made_public_before)
          # read in the data
...
protected
  ...
  def self.parse_query_params(with_laws=false, with_law_vote_summary=false, passed_after=nil, passed_before=nil, made_public_after=nil, made_public_before=nil, parliament_id=nil)
    ...
    q << "_parl_#{parliament_id}" if parliament_id.present?
    return q
  end
