class AdminController < ApplicationController
  ...
  before_filter :only => [:delete_file, :process_file, :add_url, :add_vote, :edit_vote, :is_law, :not_law, :edit_agenda, :edit_conference, :laws, :session_match, :make_public, :unpublish] do |controller_instance|
  ...
  def unpublish
    ...
    if @agenda.present?
      @agenda.is_public = false
      if @agenda.save
        flash[:notice] = t('app.msgs.law_is_not_public', :name => @agenda.official_law_title)
      else
        flash[:error] = '<ul>'.html_safe
        @agenda.errors.full_messages.each_with_index do |msg, i|
          flash[:error] << "<li>".html_safe
