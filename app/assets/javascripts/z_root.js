function set_dates_for_parl_selection(start_ary, end_ary){
  var start, end;
...
$(document).ready(function(){
    ...
    $.datepicker.setDefaults( $.datepicker.regional[ locale ] );
        ...
        changeMonth: true,
            ...
            // update data table
            if ($('#passed_laws_datatable').length > 0){
              passed_laws_dt.fnDraw();
            }
            if ($('#member_votes_datatable').length > 0){
              member_votes_dt.fnDraw();
            }
				}
		});
		$('#end_date').datepicker({
    ...
    $('input[name="parliament[]"]').click(function(){
      var start_ary = [];
      var end_ary = [];
      $('input[name="parliament[]"]:checked').each(function(){
        start_ary.push($(this).data('start'));
        end_ary.push($(this).data('end'));
      });
      set_dates_for_parl_selection(start_ary, end_ary);
    });
